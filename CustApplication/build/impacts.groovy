 ...
 * successful CustApplication build. The result of the script is the generation of     a build list text
 ...
 * CustApplication build.groovy script.
 *
 * If the --buildHash option is omitted or if no previous successful CustApplication build exists with a
 * buildHash property, the contents of CustApplication/build/files.t    xt will be copied to ${workDir}/buildList.txt
 * resulting in a full build.
 *
 * usage:  impacts.groovy [options]
 *
 *  **NOTE - CustApplication/build/build.properties will be loaded (if exists) at script startup.

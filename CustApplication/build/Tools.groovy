...
def loadProperties(OptionAccessor opts) {
	...
	// load datasets.properties containing system specific PDS names used by Cust Application build
	...
	// load bind.properties containing DB2 BIND PACKAGE parameters used by Cust Application build
	properties.load(new File("${getScriptDir()}/bind.properties"))
	// load bindlinkEditScanner.properties containing Link Edit scanning options used by Cust Application build
...
def getDefaultDependencyResolver(String file) {
    def properties = BuildProperties.getInstance()
	def path = new DependencyPath().sourceDir(properties.sourceDir).directory("CustApplication/COPY")
...
def getUserBuildDependencyResolver(String file, String relativePath) {
    ...
    if (properties.sourceDir.endsWith("Build")) {
        if (relativePath.endsWith("Build//Build")) {
            path = new DependencyPath().sourceDir(properties.sourceDir).directory("CustApplication/COPY")
        }
        else {
  	    path = new DependencyPath().sourceDir(properties.sourceDir).directory("CustApplication/COPY")
        }
...
def getDefaultImpactResolver(String file) {
	...
	// Add rule to assocuate changes to copybooks with the need to recompile programs that reference them.
   	def cpypath = new DependencyPath().collection(properties.collection).sourceDir(properties.sourceDir).directory("CustApplication/COPY")
	...
	// Add rule to associate changes to BMS maps with need to recompile programs that use the generated copybook.
   	def bmspath = new DependencyPath().collection(properties.collection).sourceDir(properties.sourceDir).directory("CustApplication/BMS")
	...
	// Add rule to associate changes to COBOL source with the need to linkedit .lnk files that reference them.
   	def cobpath = new DependencyPath().collection(properties.collection).sourceDir(properties.sourceDir).directory("CustApplication/COBOL")
